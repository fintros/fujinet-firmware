...
// compiler knows this is not the driver and gives warnings on interrupt mask enable/disable
    ...
    for(i = 0; i < 32; i++) {
        if(i < 24) {
                ...
                switch(i % 3) {
                    ...
                    case 0:
                        ...
                        if(rmt_tx_end_callback.function != NULL) {
                            rmt_tx_end_callback.function((rmt_channel_t)channel, rmt_tx_end_callback.arg);
                        }
                        break;
                        //RX_END
                    case 1:
                        RMT.conf_ch[channel].conf1.rx_en = 0;
                        item_len = rmt_get_mem_len((rmt_channel_t)channel);
            ...
            if(intr_st & (BIT(i))) {
                ...
                {
                // this is the code I need to change to make it do continuous transmission
                // some analysis: first check to see if there's a translator function:
                    if (p_rmt->translator)
                    {
                        // then look to see if there's data left to feed
                        if (p_rmt->sample_size_remain > 0)
                        {
                            ...
                            if (p_rmt->sample_size_remain == 0 && !p_rmt->wait_done)
                            {
                                ...
                                // reset p_rmt->sample_size_remain to the size of the source buffer
                                p_rmt->sample_size_remain = p_rmt->sample_size_total;
                                // if we only filled a partial buffer (tx_len_rem < tx_sub_len) ...
                                if (p_rmt->tx_len_rem < p_rmt->tx_sub_len)
                                {   
                                    size_t last_len = p_rmt->tx_len_rem;
                                    // ... then fill up the rest
                                    p_rmt->sample_to_rmt((void *)p_rmt->sample_cur,             // point to the source buffer, which we reset already
                                                         p_rmt->tx_buf + p_rmt->tx_len_rem,     // point to the destination buffer, except have to offset by the amount we already translated
                                    ...
                                    // move the pointer over
                                    p_rmt->sample_cur += translated_size;
                                    p_rmt->tx_len_rem += last_len;
                                }
                            }
                        }
                        else
                        {
                            // if there's no data left, turn things off
                            p_rmt->sample_cur = NULL;
                            p_rmt->translator = false;
                        }
                    }
                    ...
                    // figure out if there's more than half a buffer left
                    ...
                    // switch the offset back and forth so figuring out which half of the transmit buffer to fill
